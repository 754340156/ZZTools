...
//自定义 图标型按钮 初始化方法
- (instancetype)initWithFrame:(CGRect)frame
             normalImageNames:(NSArray *)normalImageNames
                  ...
                  imageHeight:(CGFloat)imageHeight
{
    ...
    if (self)
    {
        ...
        //循环创建buttons
        NSInteger count = normalImageNames.count;
        _btnCount = count;
        CGFloat btnWidth = frame.size.width / count;
        _btnWidth = btnWidth;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            [btn setImage:[UIImage imageNamed:normalImageNames[i]] forState:UIControlStateNormal];
                  ...
                  imageHeight:(CGFloat)imageHeight
{
    ...
    if (self)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            btn.frame = CGRectMake(i * btnWidth, 0, btnWidth, selfHeight);
            [btn setImage:[UIImage imageNamed:normalImageNames[i]] forState:UIControlStateNormal];
            [btn setImage:[UIImage imageNamed:selectedImageNames[i]] forState:UIControlStateSelected];
            btn.imageEdgeInsets = UIEdgeInsetsMake((selfHeight - imageHeight) / 2.0, (btnWidth - imageWidth) / 2.0, (selfHeight - imageHeight) / 2.0, (btnWidth - imageWidth) / 2.0);
            btn.tag = 101 + i;
                        ...
                        space:(CGFloat)space
{
    ...
    if (self)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            CustomButton *btn = [[CustomButton alloc] initWithFrame:CGRectMake(i * btnWidth, 0, btnWidth, selfHeight) style:style layoutStyle:CustomButtonLayoutStyleNone font:[UIFont systemFontOfSize:11] title:btnTitles[i] image:[UIImage imageNamed:normalImageNames[i]] space:space margin:0 autoSize:NO];
