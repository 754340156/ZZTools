...
- (instancetype)initWithFrame:(CGRect)frame title:(NSString *)title allArray:(NSArray *)allArray selectedArray:(NSArray *)selectedArray isMuti:(BOOL)isMuti configureBlcok:(TagBtnsViewConfigureBlock)configureBlcok selectedBlock:(TagBtnsViewSelectedBlock)block
{
    ...
    if (self)
    {
        ...
        for (int i = 0; i < allArray.count; i++)
        {
            ...
            CustomButton *tagBtn = [[CustomButton alloc] initWithFrame:CGRectMake(tagBtnLeft, tagBtnTop, tagBtnWidth, _btnsHeight) style:CustomButtonStyleNormal layoutStyle:CustomButtonLayoutStyleNone font:_btnTitleFont title:btnTitle image:nil space:0 margin:0 autoSize:YES];
...
- (instancetype)initWithFrame:(CGRect)frame btnLayoutStyle:(CustomButtonLayoutStyle)btnLayoutStyle title:(NSString *)title allArray:(NSArray *)allArray selectedArray:(NSArray *)selectedArray btnCountALine:(NSUInteger)btnCountALine normalImage:(UIImage *)normalImage selectedImage:(UIImage *)selectedImage titleAndImageSpace:(CGFloat)space isMuti:(BOOL)isMuti configureBlcok:(TagBtnsViewConfigureBlock)configureBlcok selectedBlock:(TagBtnsViewSelectedBlock)block
{
    self = [super initWithFrame:frame];
    if (self)
    {
        ...
        for (int i = 0; i < allArray.count; i++)
        {
            ...
            CustomButton *tagBtn = [[CustomButton alloc] initWithFrame:CGRectMake(tagBtnLeft, tagBtnTop, tagBtnWidth, _btnsHeight) style:CustomButtonStyleNormal layoutStyle:btnLayoutStyle font:_btnTitleFont title:btnTitle image:_btnNormalImage space:space margin:0 autoSize:YES];
            tagBtn.tag = kTag_Btn + i;
            tagBtn.normalBgColor = _btnNormalBgColor;
            tagBtn.selectedBgColor = _btnSelectedBgColor;
            [tagBtn setTitleColor:_btnTitleNormalColor forState:UIControlStateNormal];
