...
+ (NSString *)getFileDocumentPath:(NSString *)fileName
{
    ...
    NSString *documentDirectory = [ZZFileDataHandle getDocumentPath];
...
+ (NSString *)getFileCachePath:(NSString *)fileName
{
    ...
    NSString *cacheDirectory = [ZZFileDataHandle getCachePath];
...
{
    if ([NSString StringIsNULL:fileName])
    {
        return nil;
    }
    // 获取资源目录路径
...
+ (BOOL)isExistFileInDocument:(NSString *)fileName
{
    ...
    NSString *filePath = [ZZFileDataHandle getFileDocumentPath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)isExistFileInCache:(NSString *)fileName
{
    ...
    NSString *filePath = [ZZFileDataHandle getFileCachePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if ([NSString StringIsNULL:aFolderNameInDoc])
    {
        return YES ;
    }
    NSString *filePath = [ZZFileDataHandle getFileDocumentPath:aFolderNameInDoc];
    if (nil == filePath)
    {
        return YES;
    }
...
{
    if ([NSString StringIsNULL:aFolderNameInCahe])
    {
        return YES ;
    }
    
    if (![ZZFileDataHandle isExistFileInCache:aFolderNameInCahe]) {
        return YES;
    }
    
    NSString *filePath = [ZZFileDataHandle getFileCachePath:aFolderNameInCahe];
    if (nil == filePath)
    {
        return YES;
    }
...
//{
//    NSString *filePath = [ZZFileDataHandle getComicBookDir:bookId];
//    if (nil == filePath)
//    {
//        return YES;
//    }
...
+ (BOOL)isExistFileInResource:(NSString *)fileName
{
    ...
    NSString *filePath = [ZZFileDataHandle getFileResourcePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if ([NSString StringIsNULL:aFilePath])
    {
        return NO;
    }
    NSFileManager *fileManager = [NSFileManager defaultManager];
...
{
    if ([NSString StringIsNULL:resourceName])
    {
        return NO;
    }
    //获取资源文件的存放目录进行
    NSString *resourcePath = [ZZFileDataHandle getFileResourcePath:resourceName];
    NSString *documentPath = [ZZFileDataHandle getFileDocumentPath:resourceName];
    ...
    if ([ZZFileDataHandle isExistFile:documentPath])
    {
        // 如果文件已存在， 那么先删除原来的
        [ZZFileDataHandle deleteFileAtPath:documentPath];
    }
...
{
    if ([NSString StringIsNULL:filePath])
    {
        return NO;
    }
    // 判断文件是否存在
...
+ (BOOL)createDirectoryAtDocument:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [ZZFileDataHandle getFileDocumentPath:dirName];
...
+ (BOOL)createDirectoryAtCache:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [ZZFileDataHandle getFileCachePath:dirName];
    if ([fileManager fileExistsAtPath:dirPath])
    {
        return YES;
    }
...
+ (BOOL)createDirectoryAtTemporary:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *tempPath = [ZZFileDataHandle getTemporaryPath];
...
+ (NSString *)reCorrentPathWithPath:(NSString *)path
{
    ...
    NSString *docPath = [ZZFileDataHandle getDocumentPath];
    ...
    if (range.length <= 0)
    {
        ...
        if (docRange.length > 0)
        {
            ...
            NSString *newPath = [ZZFileDataHandle getFileDocumentPath:relPath];
            return newPath;
        }
    }
    return path;
...
{
    return [ZZFileDataHandle getTemporaryPath];
}
@end
