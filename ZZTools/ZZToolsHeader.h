//
//  ZZToolsHeader.h
//  ZZTools
//
//  Created by zhaozhe on 16/12/6.
//  Copyright © 2016年 zhaozhe. All rights reserved.
//

#ifndef ZZToolsHeader_h
#define ZZToolsHeader_h
#import "PhotosCollectionView.h"//上传多图
#import "ZZFileDataHandle.h"//文件句柄,单例类做缓存计算和清除
#import "ZZMBProgressManager.h"
#import "ZZCustomAlertView.h"
#import "CodeButton.h"// 获取验证码的button
#import "ButtonsView.h"//  横条排布多个按钮可带下线
#import "TagBtnsView.h"  //按钮标签视图
#import "GuidePagesHelper.h"//引导页
#import "AdvertiseHelper.h"//广告页
#import "DatePickerView.h"//时间选择器
#import "AddressPickerView.h"//省市区地址选择
#import "CustomSwitch.h" //自定义switch
#import "CustomButton.h"//自定义button布局
#import "Singleton.h" //单利宏
#import "ZZAliyunTools.h" //阿里云工具
#import "ZZProgressView.h" //cell上自定义进度条
#import "ZZButtonsView.h"//横条排布多个按钮可带下线
#import "ZZActivityView.h"//自定义加载视图
#import "ZZHeader.h"
#endif /* ZZToolsHeader_h */
